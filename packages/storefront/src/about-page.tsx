...
const institutionalImages = {
  hero: "https://static.wixstatic.com/media/8cfa65_dc4e5b209a284996b6cec4339116da28f000.jpg/v1/fill/w_890,h_1342,al_c,q_85/8cfa65_dc4e5b209a284996b6cec4339116da28f000.jpg",
...
const content = {
  brindes: { eyebrow: "Lenterne · fabricação própria", headline: "Mais de 10 anos transformando ideias em produtos.", intro: "Qualidade, personalização e atendimento próximo, da escolha do produto à entrega final.", storyTitle: "Experiência de fábrica em cada pedido.", story: "O grupo Lenterne se consolidou no fornecimento e na fabricação de acessórios para cordões de crachá, ferragens e produtos plásticos. Em Taboão da Serra, combinamos experiência, tecnologia e acompanhamento direto para atender empresas, eventos e ações promocionais.", capabilities: ["Produção própria", "Personalização", "Conferência de arte", "Atendimento direto"], closing: "Um pedido bem produzido começa com uma conversa clara." },
  ferragens: { eyebrow: "Lenterne · fabricação e fornecimento", headline: "Componentes direto de quem conhece a produção.", intro: "Há mais de 10 anos atendemos montagens, revendas e compras recorrentes com orientação técnica próxima.", storyTitle: "Da ferragem de linha ao desenvolvimento plástico.", story: "Fornecemos argolas, garras e clips para chaveiros, cordões e identificação. Também desenvolvemos projetos, moldes de injeção plástica e serviços por hora-máquina conforme a necessidade de cada operação.", capabilities: ["Atacado e revenda", "Compras recorrentes", "Moldes de injeção", "Hora-máquina"], closing: "A peça certa mantém a sua produção em movimento." }
} as const;
...
export function AboutPage({ brand }: { brand: BrandConfig }) {
  ...
  return <main id="conteudo" className={`about-page ${brand.channel}-about`}>
    <section className="about-hero"><MotionBackdrop /><div className="shell about-hero-inner"><div><p className="eyebrow">{copy.eyebrow}</p><h1>{copy.headline}</h1><p>{copy.intro}</p><a className="button button-primary" href="/orcamento">Falar com a Lenterne <Arrow /></a></div><figure><img src={brand.channel === "brindes" ? institutionalImages.hero : items[0].image} alt={brand.channel === "brindes" ? "Produção e produtos da Lenterne" : items[0].imageAlt} /></figure></div></section>
    <section className="about-story shell"><p className="eyebrow">Quem somos</p><div><h2>{copy.storyTitle}</h2><p>{copy.story}</p></div></section>
    <section className="about-capabilities shell" aria-label="Capacidades da Lenterne">{copy.capabilities.map((item, index) => <div key={item}><span>{String(index + 1).padStart(2, "0")}</span><strong>{item}</strong></div>)}</section>
    ...
    {brand.channel === "brindes" && <section className="about-showcase shell"><header><p className="eyebrow">O que produzimos</p><h2>Produtos pensados para acompanhar pessoas e marcas.</h2></header><div>{showcase.map((item) => <article key={item.title}><figure><img src={item.image} alt={item.title} loading="lazy" /></figure><div><span>{item.tag}</span><h3>{item.title}</h3><p>{item.text}</p><a href="/catalogo">Ver no catálogo <Arrow /></a></div></article>)}</div></section>}
